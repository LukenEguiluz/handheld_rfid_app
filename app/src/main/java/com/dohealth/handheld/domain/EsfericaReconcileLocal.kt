...
object EsfericaReconcileLocal {
    ...
    /** RFIDs únicos leídos que no están en el catálogo esperado, en orden de primera lectura. */
    fun offCatalogRfidsOrdered(
        expectedItems: List<ExpectedInventoryItemDto>,
        scannedRfidsOrdered: List<String>,
    ): List<String> {
        val expected = normalizedExpectedRfidSet(expectedItems)
        return distinctNormalizedPreserveOrder(scannedRfidsOrdered).filter { it !in expected }
    }
